...
from m2isar.metamodel.utils.expr_preprocessor import (process_attributes,
                                                  process_functions,
...
def main():
  ...
  # TODO: filter builtins/aliases
	...
	def check_filter(name, keep, drop):
		...
		return True
	model["sets"] = {set_name: set_def for set_name, set_def in model["sets"].items() if check_filter(set_name, keep_sets, drop_sets)}
	for set_name, set_def in model["sets"].items():
		set_def.instructions = {instr_name: instr_def for instr_name, instr_def in set_def.instructions.items() if check_filter(instr_name, keep_instructions, drop_instructions)}
