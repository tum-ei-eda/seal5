...
def main():
	...
	for set_name, set_def in model["sets"].items():
		...
		if len(context.to_drop) > 0:
			...
			set_def.constants = {const_name: const for const_name, const in set_def.constants.items() if const_name not in context.to_drop}
		...
		if len(context.to_drop) > 0:
			...
			set_def.memories = {mem_name: mem for mem_name, mem in set_def.memories.items() if mem_name not in context.to_drop}
