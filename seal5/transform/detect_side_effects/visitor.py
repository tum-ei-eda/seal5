...
"""Detect memory side effects in instruction behavior."""
...
class DetectSideEffectsVisitor(ExprVisitor):
    """Set memory side-effect flags in the instruction context."""
    ...
    def _(self, expr: behav.IndexedReference, context):
        if isinstance(expr.reference, arch.Memory):
            if arch.MemoryAttribute.IS_MAIN_MEM in expr.reference.attributes:
                if context.is_write:
