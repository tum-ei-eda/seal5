...
def main():
	...
	for set_name, set_def in sorted(models["sets"].items()):
		...
		for mem_name, mem_def in sorted(set_def.memories.items()):
			if args.text:
				_ = Node(f"{mem_name}", parent=mems_node, value=f"{mem_def.range.upper}:{mem_def.range.lower} ({mem_def.range.length}), {mem_def.size}")
			else:
		...
		for mem_name, mem_def in sorted(set_def.memory_aliases.items()):
			if args.text:
				_ = Node(f"{mem_name} ({mem_def.parent.name})", parent=alias_node, value=f"{mem_def.range.upper}:{mem_def.range.lower} ({mem_def.range.length}), {mem_def.size}")
			else:
