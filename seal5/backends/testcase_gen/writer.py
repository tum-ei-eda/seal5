...
def write_inline_asm_c_test(
    ...
    with open(output_path / f"{mnemonic.lower()}.test-inline-asm.c", "w", encoding="utf-8") as f:
        f.write(txt)
...
def write_intrin_ll_test(instr_name, mnemonic, xlen, output_path, set_name: str, start_time: str):
    ...
    with open(output_path / f"{mnemonic.lower()}.test-intrin.ll", "w", encoding="utf-8") as f:
        f.write(txt)
    ...
    if args.splitted:
        ...
        for set_name, set_def in model_obj.sets.items():
            ...
            for _, instr_def in set_def.instructions.items():
                ...
                output_file = f"{set_dir / out_name}"
                try:
                    metrics["n_success"] += 1
