...
def explicit_extends(
    ...
    **_kwargs,
    ...
    assert input_file.is_file(), f"File not found: {input_file}"
    name = input_file.name
    logger.info("Adding excplicit extends for %s", name)
    args = [
        settings.models_dir / name,
        "--log",
        log_level,
    ...
    if gen_metrics_file:
        # TODO: move to .seal5/metrics
        metrics_file = settings.temp_dir / (name + "_explicit_extends_metrics.csv")
        args.extend(["--metrics", metrics_file])
    if not use_subprocess:
        from seal5.transform.explicit_extends import ExplicitExtends

        args = sanitize_args(args)
        ExplicitExtends(args)
    else:
        utils.python(
            "-m",
            "seal5.transform.explicit_extends.transform",
            *args,
            env=env,
            print_func=logger.info if verbose else logger.debug,
            live=True,
...
def explicit_register_names(
    ...
    **_kwargs,
    ...
    assert input_file.is_file(), f"File not found: {input_file}"
    name = input_file.name
    logger.info("Adding excplicit register names for %s", name)
    args = [
        settings.models_dir / name,
        "--log",
        log_level,
    ...
    if gen_metrics_file:
        # TODO: move to .seal5/metrics
        metrics_file = settings.temp_dir / (name + "_explicit_register_names_metrics.csv")
        args.extend(["--metrics", metrics_file])
    if not use_subprocess:
        from seal5.transform.explicit_register_names import ExplicitRegisterNames

        args = sanitize_args(args)
        ExplicitRegisterNames(args)
    else:
        utils.python(
            "-m",
            "seal5.transform.explicit_register_names.transform",
            *args,
            env=env,
            print_func=logger.info if verbose else logger.debug,
            live=True,
