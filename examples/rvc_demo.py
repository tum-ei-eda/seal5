...
if PREPATCHED:
    if seal5_flow.repo is None or "seal5-rvc-stage0" not in seal5_flow.repo.tags:
...
seal5_flow.initialize(
    ...
    clone_ref="seal5-rvc-stage0" if PREPATCHED else "llvmorg-18.1.0-rc3",
...
# Load CoreDSL inputs
cdsl_files = [
    EXAMPLES_DIR / "cdsl" / "rv_example" / "ExampleRV32C.core_desc",
]
seal5_flow.load(cdsl_files, verbose=VERBOSE, overwrite=True)

# Load test inputs
test_files = [
    EXAMPLES_DIR / "tests" / "xrvc" / "*.s",
    EXAMPLES_DIR / "tests" / "xrvc" / "*.ll",
    EXAMPLES_DIR / "tests" / "xrvc" / "*.c",
]
