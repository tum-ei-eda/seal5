...
int __builtin_xchacha_xorrol7(int, int);

int test_xorrol7(int a, int b) {
    // CHECK-OBJ: <test_xorrol7>
    // CHECK-OBJ-NEXT: chacha.xorrol7
    // CHECK-LL-LABEL: @test_xorrol7
    // CHECK-LL-NEXT:    [[TMP0:%.*]] = tail call i32 @llvm.riscv.xchacha.xorrol7(i32 %0, i32 %1)
    // CHECK-LL-NEXT:    ret i32 [[TMP0]]
    return __builtin_riscv_xchacha_xorrol7(a, b);
}
